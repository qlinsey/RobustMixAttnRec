from mixtime.loggers import *

# from config import STATE_DICT_KEY, OPTIMIZER_STATE_DICT_KEY, TRAIN_LOADER_RNG_STATE_DICT_KEY
from mixtime.config import *
from mixtime.utils import AverageMeterSet
from mixtime.utils import fix_random_seed_as
from mixtime.analyze_table import find_saturation_point
...
class AbstractTrainer(metaclass=ABCMeta):
    def __init__(
        self, args, model, train_loader, val_loader, test_loader, local_export_root
    ):
        self.args = args
        ...
        self.train_loggers, self.val_loggers, self.test_loggers = (
            self._create_loggers() if not self.pilot else (None, None, None)
        )
        self.add_extra_loggers()

        self.logger_service = LoggerService(
            args, self.train_loggers, self.val_loggers, self.test_loggers
        )
    ...
    def train(self):
        ...
        for epoch in range(self.epoch_start, self.num_epochs):
            ...
            fix_random_seed_as(
                epoch
            )  # fix random seed at every epoch to make it perfectly resumable
            ...
            if stop_training:
                ...
                self.validate(
                    best_epoch, accum_iter, mode="test"
                )  # test result at best model
                break
    ...
    def _create_loggers(self):
        train_table_definitions = [("train_log", ["epoch", "loss"])]
        ...
        train_loggers = [
            TableLoggersManager(
                args=self.args,
                export_root=self.local_export_root,
                table_definitions=train_table_definitions,
            )
        ]
        val_loggers = [
            TableLoggersManager(
                args=self.args,
                export_root=self.local_export_root,
                table_definitions=val_table_definitions,
            )
        ]
        test_loggers = [
            TableLoggersManager(
                args=self.args,
                export_root=self.local_export_root,
                table_definitions=test_table_definitions,
            )
        ]
        ...
        if USE_WANDB:
            ...
            val_loggers.append(
                WandbLogger(table_definitions=val_table_definitions, prefix="val_")
            )
            test_loggers.append(
                WandbLogger(table_definitions=test_table_definitions, prefix="test_")
            )
    ...
    def _create_state_dict(self, epoch, accum_iter):
        return {
            STATE_DICT_KEY: self.model.module.state_dict()
            if self.use_parallel
    ...
    def _restore_best_state(self):
        ...
        sat, reached_end = find_saturation_point(
            df, self.saturation_wait_epochs, display=False
        )
        e = sat["epoch"].iloc[0]
        self.best_epoch = e
        ...
        state_dict_path = os.path.join(
            self.local_export_root, "models", BEST_STATE_DICT_FILENAME
        )
    ...
    def _restore_training_state(self):
        ...
        ###
        state_dict_path = os.path.join(
            self.local_export_root, "models", RECENT_STATE_DICT_FILENAME
        )
        chk_dict = torch.load(os.path.abspath(state_dict_path))
        ...
        train_loader_dataset_rng_state = chk_dict[
            TRAIN_LOADER_DATASET_RNG_STATE_DICT_KEY
        ]
        self.train_loader.dataset.set_rng_state(train_loader_dataset_rng_state)
        train_loader_sampler_rng_state = chk_dict[
            TRAIN_LOADER_SAMPLER_RNG_STATE_DICT_KEY
        ]
