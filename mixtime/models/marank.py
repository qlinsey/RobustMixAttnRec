...
class MARankModel(BaseModel):
    def __init__(self, args):
        ...
        self.ures_linear_layers = nn.ModuleList(
            [nn.Linear(hidden, hidden) for _ in range(self.num_att_layers)]
        )
        self.ires_linear_layers = nn.ModuleList(
            [nn.Linear(hidden, hidden) for _ in range(self.num_att_layers)]
        )
        self.res_linear_layers = nn.ModuleList(
            [nn.Linear(hidden, hidden) for _ in range(self.num_linear_layers)]
        )
        ...
        self.rel_att_item = nn.ModuleList(
            [nn.Linear(hidden, hidden) for _ in range(self.num_att_layers)]
        )
        self.rel_att_user = nn.ModuleList(
            [nn.Linear(hidden, hidden) for _ in range(self.num_att_layers)]
        )
        self.rel_att_out = nn.ModuleList(
            [nn.Linear(hidden, 1) for _ in range(self.num_att_layers)]
        )
