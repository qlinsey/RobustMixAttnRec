...
class TiSasModel(BaseModel):
    ...
    def forward(self, d):
        ...
        if self.training:
            ...
            candidate_embeddings = self.token_embedding.emb(
                d["candidates"]
            )  # B x C x H
    ...
    def get_logits(self, d):
        ...
        relative_time_key = self.dropout(
            self.relative_time_key_embedding(d)
        )  # B x T x H
        relative_time_value = self.dropout(
            self.relative_time_value_embedding(d)
        )  # B x T x H
    ...
    def get_loss(self, logits, labels, negative_labels):
        ...
        valid_negative_labels_emb = self.token_embedding.emb(
            valid_negative_labels
        )  # M x H

        valid_labels_prob = self.sigmoid((valid_logits * valid_labels_emb).sum(-1))  # M
        valid_negative_labels_prob = self.sigmoid(
            (valid_logits * valid_negative_labels_emb).sum(-1)
        )  # M

        loss = -torch.log(valid_labels_prob + 1e-24) - torch.log(
            (1 - valid_negative_labels_prob) + 1e-24
        )
        loss = loss.mean()
