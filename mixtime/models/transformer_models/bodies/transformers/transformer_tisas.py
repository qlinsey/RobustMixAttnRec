...
class TisasTransformerBlock(nn.Module):
    def __init__(self, args):
        ...
        self.feed_forward = PositionwiseFeedForward(
            d_model=hidden, d_ff=feed_forward_hidden, dropout=dropout, act="relu"
        )
    ...
    def forward(self, x, mask, pos_k, pos_v, r_k, r_v, layer, info):
        x = self.input_sublayer(
            x,
            lambda _x: self.attention.forward(
                _x, _x, _x, mask, pos_k, pos_v, r_k, r_v, layer, info
            ),
        )
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask, pos_k, pos_v, r_k, r_v, layer, info):
        ...
        # B x n x T x d
        pos_k, pos_v = [
            x.view(batch_size, T, self.h, self.d_k).transpose(1, 2)
            for x in [pos_k, pos_v]
        ]

        # B x n x T x T x d
        r_k, r_v = [
            x.view(batch_size, T, T, self.h, self.d_k).permute(0, 3, 1, 2, 4)
            for x in [r_k, r_v]
        ]

        # x : B x n x T x d
        x, attn = self.attention(
            query, key, value, mask, pos_k, pos_v, r_k, r_v, layer, info
        )
