from mixtime.models.transformer_models.utils import (
...
class TransformerMixtimeBlock(nn.Module):
    def __init__(self, args, La, Lr):
        ...
        self.input_sublayer = SublayerConnection(
            args=args, size=hidden, dropout=dropout
        )
    ...
    def forward(self, x, mask, abs_kernel, rel_kernel, layer, info):
        ...
        x = self.input_sublayer(
            ...
            lambda _x: self.attention(
                _x, _x, _x, mask, abs_kernel, rel_kernel, layer, info
            ),
        )
...
class MixedAttention(nn.Module):
    def __init__(self, args, La, Lr):
        ...
        ## TODO
        self.content_linear_layers = nn.ModuleList(
            [nn.Linear(d_model, d_model) for _ in range(3)]
        )
        self.abs_position_query_linear_layers = nn.ModuleList(
            [nn.Linear(d_model, self.d_k) for _ in range(La)]
        )
        self.abs_position_key_linear_layers = nn.ModuleList(
            [nn.Linear(d_model, self.d_k) for _ in range(La)]
        )
        self.rel_position_key_linear_layers = nn.ModuleList(
            [nn.Linear(d_model, self.d_k) for _ in range(Lr)]
        )
        ...
        self.rel_position_bias = nn.Parameter(
            torch.FloatTensor(1, self.Lr, 1, self.d_k)
        )
        ## OUTPUT
    ...
    def forward(self, query, key, value, mask, abs_kernel, rel_kernel, layer, info):
        ...
        if self.La > 0:
            ...
            Pk = torch.stack(
                [l(x) for l, x in zip(self.abs_position_key_linear_layers, abs_kernel)],
                dim=1,
            )  # B x La x T x d
            abs_scores = torch.einsum(
                "blid,bljd->blij", Xq + Pq, Xk + Pk
            )  # B x La x T x T
        ...
        if self.Lr > 0:
            ...
            R = torch.stack(
                [l(x) for l, x in zip(self.rel_position_key_linear_layers, rel_kernel)],
                dim=1,
            )  # B x Lr x T x T x d
            ...
            rel_scores += torch.einsum(
                "blid,blijd->blij", Xq + self.rel_position_bias, R
            )  # B x Lr x T x T
        ...
        x = (
            x.transpose(1, 2).contiguous().view(batch_size, T, self.h * self.d_k)
        )  # B x T x H
