...
class BertLinearPredictionHead(nn.Module):
    def __init__(self, args, input_size=None):
        ...
        if args.head_use_ln:
            ...
            self.out = nn.Sequential(
                nn.Linear(hidden, hidden), GELU(), nn.Linear(hidden, self.vocab_size)
            )
...
class BertDotProductPredictionHead(nn.Module):
    def __init__(self, args, token_embeddings, input_size=None):
        ...
        if args.head_use_ln:
            self.out = nn.Sequential(
                nn.Linear(input_size, hidden), GELU(), nn.LayerNorm(hidden)
            )
        else:
            self.out = nn.Sequential(
...
class BertL2PredictionHead(nn.Module):
    def __init__(self, args):
        ...
        self.vocab_size = args.num_items + 1
        self.out = nn.Sequential(
            nn.Linear(hidden, hidden), GELU(), nn.LayerNorm(hidden)
        )
    ...
    def forward(self, x, token_embeddings):
        ...
        emb = (
            token_embeddings.weight[: self.vocab_size].unsqueeze(0).unsqueeze(1)
        )  # 1 x 1 x V x H
