from mixtime.models import model_factory
from mixtime.dataloaders import dataloader_factory
from mixtime.trainers import trainer_factory
from mixtime.utils import *
...
def train(args):
    local_export_root, remote_export_root, communicator = setup_train(
        args, MACHINE_IS_HOST
    )
    assert (communicator is None and MACHINE_IS_HOST) or (
        communicator is not None and not MACHINE_IS_HOST
    )
    if communicator:
    ...
    trainer = trainer_factory(
        args, model, train_loader, val_loader, test_loader, local_export_root
    )
