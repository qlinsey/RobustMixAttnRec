...
from mixtime.communicator import Communicator
    ...
    if machine_is_host:
        ...
        if not created:
            ...
            if status == STATUS_RECOVERY:
                ...
                shutil.rmtree(local_export_root)
                print(
                    "Remote export root {} exists. Existing".format(remote_export_root)
                )
                exit(0)
        else:
...
def sameDirsAux(cmp):
    return len(cmp.diff_files) == 0 and all(
        sameDirsAux(c) for c in cmp.subdirs.values()
    )
