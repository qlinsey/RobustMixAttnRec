...
class AbstractDataset(metaclass=ABCMeta):
    ...
    def preprocess(self):
        ...
        user2dict, train_targets, validation_targets, test_targets = self.split_df(
            df, len(umap)
        )
    ...
    def split_df(self, df, user_count):
        def sort_by_time(d):
            ...
            return {
                "items": list(d.sid),
                "timestamps": list(d.timestamp),
                "ratings": list(d.rating),
                "days": list(d.days),
            }
    ...
    def _get_preprocessed_folder_path(self):
        ...
        folder_name = "{}_min_rating{}-min_uc{}-min_sc{}-split{}".format(
            self.code(), self.min_rating, self.min_uc, self.min_sc, self.split
        )
