...
class SasDataloader(AbstractDataloader):
    ...
    def calculate_sas_timestamps(self):
        ...
        for user, dic in self.user2dict.items():
            ...
            min_time = min(times)
            sas_timestamps[user] = [
                round((t - min_time) / time_scale) + 1 for t in times
            ]  # followed authors' original implementation
...
class SasTrainDataset(BertTrainDataset):
    def __init__(
        self, args, dataset, negative_samples, rng, train_ranges, sas_timestamps
    ):
    ...
    def __getitem__(self, index):
        ...
        if self.marank_mode:
            ...
            neg_samples = self.negative_samples[
                user
            ]  # a pool of negative items to choose from
